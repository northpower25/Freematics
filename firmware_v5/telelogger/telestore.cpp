...
/*******************************************************************************
  CStorageJSON - JSON serializer for Home Assistant webhook
*******************************************************************************/

const char* CStorageJSON::pidToKey(uint16_t pid, int idx)
{
    switch (pid) {
    case PID_TIMESTAMP:         return "ts";
    case PID_GPS_LATITUDE:      return "lat";
    case PID_GPS_LONGITUDE:     return "lng";
    case PID_GPS_ALTITUDE:      return "alt";
    case PID_GPS_SPEED:         return "gps_speed";
    case PID_GPS_HEADING:       return "heading";
    case PID_GPS_SAT_COUNT:     return "satellites";
    case PID_GPS_TIME:          return "gps_time";
    case PID_GPS_HDOP:          return "hdop";
    case PID_ACC:
        switch (idx) {
        case 0: return "acc_x";
        case 1: return "acc_y";
        ...
        break;
    case PID_BATTERY_VOLTAGE:   return "battery";
    case PID_CSQ:               return "signal";
    case PID_DEVICE_TEMP:       return "device_temp";
    // OBD mode-1 PIDs stored with 0x100 prefix
    case (0x100 | 0x04):        return "engine_load";
    case (0x100 | 0x05):        return "coolant_temp";
    case (0x100 | 0x0A):        return "fuel_pressure";
    case (0x100 | 0x0C):        return "rpm";
    case (0x100 | 0x0D):        return "speed";
    case (0x100 | 0x0E):        return "timing_advance";
    case (0x100 | 0x0F):        return "intake_temp";
    case (0x100 | 0x11):        return "throttle";
    }
    return nullptr;
}

void CStorageJSON::appendField(const char* key, const char* value)
{
    bool needComma = (m_cacheBytes > 1);
    // Pre-check: ensure enough space for comma + "key":value + null terminator
    int needed = (needComma ? 1 : 0) + 1 + (int)strlen(key) + 2 + (int)strlen(value) + 1;
    int remain = (int)(m_cacheSize - m_cacheBytes);
    if (remain < needed) return;
    if (needComma) {
        m_cache[m_cacheBytes++] = ',';
    }
    int written = snprintf(m_cache + m_cacheBytes, remain - (needComma ? 1 : 0),
        "\"%s\":%s", key, value);
    if (written > 0) {
        m_cacheBytes += written;
        m_samples++;
    }
}

void CStorageJSON::header(const char* devid)
{
    m_cacheBytes = snprintf(m_cache, m_cacheSize, "{\"device_id\":\"%s\"", devid);
}

void CStorageJSON::tailer()
{
    if (m_cacheBytes < m_cacheSize - 1) {
        m_cache[m_cacheBytes++] = '}';
        m_cache[m_cacheBytes] = 0;
    }
}

void CStorageJSON::log(uint16_t pid, uint8_t values[], uint8_t count)
{
    for (int i = 0; i < count; i++) {
        const char* key = pidToKey(pid, i);
        if (!key) continue;
        char buf[16];
        snprintf(buf, sizeof(buf), "%u", (unsigned int)values[i]);
        appendField(key, buf);
    }
}

void CStorageJSON::log(uint16_t pid, uint16_t values[], uint8_t count)
{
    for (int i = 0; i < count; i++) {
        const char* key = pidToKey(pid, i);
        if (!key) continue;
        char buf[16];
        // PID_BATTERY_VOLTAGE is stored as voltage*100 (uint16_t); convert to V
        if (pid == PID_BATTERY_VOLTAGE) {
            snprintf(buf, sizeof(buf), "%.2f", values[i] / 100.0f);
        } else {
            snprintf(buf, sizeof(buf), "%u", (unsigned int)values[i]);
        }
        appendField(key, buf);
...
void CStorageJSON::log(uint16_t pid, uint32_t values[], uint8_t count)
{
    for (int i = 0; i < count; i++) {
        const char* key = pidToKey(pid, i);
        if (!key) continue;
...
void CStorageJSON::log(uint16_t pid, int32_t values[], uint8_t count)
{
    for (int i = 0; i < count; i++) {
        const char* key = pidToKey(pid, i);
        if (!key) continue;
        char buf[16];
        snprintf(buf, sizeof(buf), "%d", (int)values[i]);
        appendField(key, buf);
    }
}

void CStorageJSON::log(uint16_t pid, float values[], uint8_t count, const char* fmt)
{
    for (int i = 0; i < count; i++) {
        const char* key = pidToKey(pid, i);
        if (!key) continue;
        char buf[24];
        snprintf(buf, sizeof(buf), fmt ? fmt : "%f", values[i]);
        appendField(key, buf);
    }
}
